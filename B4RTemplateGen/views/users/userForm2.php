<?php
//-----------------------------PAGE FOR SECOND LAYOUT: HARVEST LABEL---------------------------//
	use yii\helpers\Html;
	use yii\helpers\Url;
	use yii\widgets\ActiveForm;
	$this->title = 'Harvest Label';
?>
<?php 
...
<!------------------SIDE BAR--------------->
	...
	<div id = "main_edit">
	<!------------------------TEXT FIELDS: FETCH DATA FROM USER-------------------------------->
	 <?php $form = ActiveForm::begin([
			'id' => 'select-form',
			'method' => 'get',
			'action' => Url::to(['users/user_secondinput']),
			]); 
	?>
	
	<?= Html::img('@web/img/labels/harvestlabels2.png', ['id' => 'harvestlabel', 'alt'=>'some', 'width'=>300]);?> 
	<input id = "first_text" name = "designation" type = "text" placeholder = "Designation">
	<input id = "second_text" name = "current_gen" type = "text" placeholder = "Current Generation">
	<input id = "third_text" name = "source" type = "text" placeholder = "Seed Source">
	<input id = "fifth_text" name = "tray_number"type = "text" placeholder = "Tray Number">
	</div>
	<input type = "number" name = "num_cop" id = "generate1" placeholder = "Number Of Copies"/>
	<?= Html::submitButton('GENERATE', ['id'=>'generate','class' => 'btn btn-info btn-lg', 'name' => 'Submit']) ?>
</div>
</form>
</html>
